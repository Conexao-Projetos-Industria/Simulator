...
public:
        ...
        tcp::resolver::results_type results)
    {
        ...
        host_ = results.begin()->host_name().c_str();
                                                   ...
                                                   std::function<void(const std::string&, const std::string&)> parserCallback) 
{
    ...
    auto const text = "\"Hello socket\"";

    // The io_context is required for all I/O
    net::io_context ioc;

    // Launch the asynchronous operation
    std::make_shared<session>(ioc)->run(activityManagerURL.c_str(), parserCallback, text);
...
void ActivityManagerConnector::AquireToken(const std::string& activityManagerURL){
    std::string url {activityManagerURL};
    tcp::iostream s(url, "http"); //TODO(giuliano) use https
...
void ActivityManagerConnector::Publish(const std::string& content, const std::string& key) {
